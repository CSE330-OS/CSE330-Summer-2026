...
check_kernel_module ()
{
    ...
    if ! check_threads ${prod} ${cons} 5; then
        let STATUS=1
    else
        KERNEL_MODULE_PTS=$(echo "$KERNEL_MODULE_PTS + $KERNEL_THREAD_POINTS" | bc)
    ...
    if ! compare_pids ${prod} ${cons} ${regular} ${zombies}
    then
        let STATUS=1
    else
        #echo "[log]: ┬─ All zombies were consumed"
	if [[ ${prod} -eq 0 ]]; then
        	echo "[log]: ┬─ Zero zombies were produced"
		echo "[log]: └─ None of the regular processes were consumed"
	else
		echo "[log]: ┬─ All zombies were produced"
		echo "[log]: └─ None of the regular processes were consumed"
	fi
    fi
    ...
    if ! check_threads 0 0 5; then
        let STATUS=1
    else
        echo "[log]: ─ All threads have been stopped"
    	KERNEL_MODULE_PTS=$(echo "$KERNEL_MODULE_PTS + $KERNEL_STOPPED_POINTS" | bc)
    fi
...
run_test ()
{
    ...
    local regular=$5
    local zombies=$6
    local dmesg_points=${7:-20}

    KERNEL_DMESG_POINTS=$((dmesg_points * 100))
    ...
    if check_kernel_module $prod $cons $size $regular $zombies; then
        echo -e "[zombie_finder]: Passed"
    else
        #echo -e "[zombie_finder]: Failed ${KERNEL_MODULE_ERR}"
        echo -e "[zombie_finder]: Failed"
    fi
...
if [ "$#" -ne 6 ] && [ "$#" -ne 7 ]; then
    echo "Usage: ./test_module.sh /path/to/your/submission/ <prod> <cons> <size> <regular> <zombies> [dmesg-points]"
    ...
    echo " <dmesg-points> - optional points assigned to dmesg log validation"
    exit 1
fi
